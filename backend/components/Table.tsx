                ...
                {data.map((row, index) => (
                    <tr key={index}>
                        ...
                        typeof row[key] === "object" && React.isValidElement(row[key]) ? (
                            ...
                            <td key={cellIndex} className={`pl-10 pr-10 p-6 ${Array.isArray(row[key]) ? row[key][1] : ''}`}>
                        ...
                        {/* when menu is open */}
                        {openMenu === index && (
                            <div className="absolute right-12 mt-1 w-28 bg-white rounded-lg shadow-[0_8px_16px_rgba(0,0,0,0.2)] z-10" onClick={(e) => e.stopPropagation()}>
                                <div className="absolute left-20 -top-3 w-0 h-0
                                    border-l-10 border-l-transparent
                                    border-r-10 border-r-transparent
                                    border-b-12 border-b-white"></div>

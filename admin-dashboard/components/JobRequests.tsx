...
import JobRequestBulkBar from "@/components/JobRequestBulkBar";
import JobRequestModal from "@/components/JobRequestModal";
import { useNotification } from "@/hooks/useNotification";

type FullJobType = Jobs;

interface JobRequestData extends DataRow {
  id: number | string;
  position: string;
...
export default function JobRequests() {
  ...
  const denyJobMutation = trpc.jobs.denyJobRequest.useMutation({
    ...
    onError: (error) => {
      console.error("Failed to deny job request:", error);
      showNotification(`Error: ${error.message}`, "error");
    },
  });
  ...
  const bulkApproveJobsMutation = trpc.jobs.bulkApproveJobRequests.useMutation({
    ...
    onError: (error) => {
      console.error("Failed to bulk approve job requests:", error);
      showNotification(`Error: ${error.message}`, "error");
    },
  });
  ...
  const bulkDenyJobsMutation = trpc.jobs.bulkDenyJobRequests.useMutation({
    ...
    onError: (error) => {
      console.error("Failed to bulk deny job requests:", error);
      showNotification(`Error: ${error.message}`, "error");
    },
  });
  ...
  const jobRequestColumns: ColumnDefinition<JobRequestData>[] = [
  ...
  const pendingJobsResourcesData: JobRequestData[] = useMemo(
    () =>
      pendingJobsData
        ?.map((job) => ({
          id: job.id,
  ...
  const reviewedJobsResourcesData: JobRequestData[] = useMemo(
    () =>
      reviewedJobsData
        ?.map((job) => ({
          id: job.id,
  ...
  const handleJobRequestRowClick = (id: number | string) => {
    const job =
      activeTabIndex === 0
  ...
  const handleJobRequestApprove = (id: number | string) => {
    approveJobMutation.mutate({ id });
  };

  const handleJobRequestDeny = (id: number | string) => {
  ...
  return (
    <div className="flex h-full w-full gap-0 relative">
      ...
      {/* Bulk Action Bar - only show for pending requests */}
      {activeTabIndex === 0 && (
        <JobRequestBulkBar
